...
class LeakyConv2D(Layer):
    def __init__(self, filters, kernel_size, strides=(1, 1), activation='relu',
                 padding='same', use_bias=True, negative_slope=0.2, **kwargs):
        super(LeakyConv2D, self).__init__(**kwargs)
        self._conv = Conv2D(filters, kernel_size, strides=strides, activation='relu',
    ...
    def get_config(self):
        config = super(LeakyConv2D, self).get_config()
        config.update({
